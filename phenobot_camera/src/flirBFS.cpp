...
void BFS_camera::streaming()
{
    ...
    if (std::filesystem::create_directory(CurrentOutputFolder)) {
        std::cout << "Folder created successfully: " << CurrentOutputFolder << std::endl;
    } else {
        std::cout << "Failed to create folder: " << CurrentOutputFolder << std::endl;
    }
...
void BFS_camera::saving()
{
    ...
    {
        cout<<"Saving starting";
        /*
        uint64_t GetUserBufferCount();
        const Spinnaker::CameraPtr pCa;
        const TransportLayerStream& camStreamInfo = pCa->TLStream;
        stringstream outSS;
    ...
    {
        cout<<"Waiting the buffer to be filled";    
    }
    else 
    */
        ...
        
        {
           ...
           // fs::path img_path = fs::path(_syncSave.save_path) / fs::path(ss_fn.str());
			...
			//if(ts_img.image_id % 3 ==0){
					...
					cout << "[" << cameraID << "] "<< ". Image saved: " << img_path_preview.c_str()<< endl;
...
int BFSMulti::BFSM_trigger(string& ts, string& ts_ros){  
    ...
    try 
        ...
        { 
            ...
            RCLCPP_ERROR_STREAM(rclcpp::get_logger("BFSMulti"),"Unable to open port /dev/ttyACM0"); 
    ...
    if(moduleMode_ == "Soft" || moduleMode_ == "GPIO"){
        ...
        // GPIO trigger
        if(moduleMode_ == "GPIO"){
        	//cout<<"enter to clear 0\n";
        	////getchar();
            //std::this_thread::sleep_for(std::chrono::milliseconds(150));
            //Param_listener::preciseSleep(0.250);
            //std::this_thread::sleep_for(std::chrono::milliseconds(9000));// 
            
            // camera:2~7, discharge:1, charge:0
            ser.write("gpio writeall fc\r"); //stop charging 1111 1100
            std::this_thread::sleep_for(std::chrono::microseconds(1000));//  1ms
            ser.write("gpio writeall 02\r"); //start discharging 0000 0010
            std::this_thread::sleep_for(std::chrono::microseconds(1000));//  500 us
            ser.write("gpio writeall fc\r"); // stop discharging 11111100
            std::this_thread::sleep_for(std::chrono::microseconds(1000));//  1ms
            ser.write("gpio writeall fd\r"); // start charging 11111101 
        	...
        	//getchar();
            //ser.write("gpio set 0\r");
            //ser.write("gpio writeall ff\r");// start
